...
class GraphGameScene1: SKScene, SKPhysicsContactDelegate {
    ...
    var backgroundLines1 =  SKSpriteNode(imageNamed: "yellowLines1")
    ...
    init(size: CGSize, viewController: GraphViewController) {
        ...
        //backgroundLines
        backgroundLines1.size = CGSizeMake(size.width, size.height)
        backgroundLines1.position = CGPoint(x: size.width/2, y: size.height/2)
        backgroundLines1.zPosition = 1
        addChild(backgroundLines1)
        ...
        two.position = CGPoint(x: width * 0.2, y: height * 0.65)
        foregroundNode.addChild(two)
        
        three = createDistanceAtPosition(.Three)
        three.position = CGPoint(x: width * 0.37, y: height * 0.65)
        foregroundNode.addChild(three)
        
        four = createDistanceAtPosition(.Three)
        four.position = CGPoint(x: width * 0.05, y: height * 0.3)
        foregroundNode.addChild(four)
        
        five = createDistanceAtPosition(.Five)
        five.position = CGPoint(x: width * 0.3, y: height * 0.85)
        foregroundNode.addChild(five)
        
        six = createDistanceAtPosition(.Five)
        six.position = CGPoint(x: width * 0.75, y: height * 0.85)
        foregroundNode.addChild(six)
        
        seven = createDistanceAtPosition(.Four)
        seven.position = CGPoint(x: width * 0.2, y: height * 0.15)
        foregroundNode.addChild(seven)
        
        eight = createDistanceAtPosition(.Four)
        eight.position = CGPoint(x: width * 0.2, y: height * 0.42)
        foregroundNode.addChild(eight)
        
        nine = createDistanceAtPosition(.Three)
        nine.position = CGPoint(x: width * 0.75, y: height * 0.64)
        foregroundNode.addChild(nine)
        
        ten = createDistanceAtPosition(.Three)
        ten.position = CGPoint(x: width * 0.75, y: height * 0.42)
        foregroundNode.addChild(ten)
        
        eleven = createDistanceAtPosition(.Three)
        eleven.position = CGPoint(x: width * 0.75, y: height * 0.15)
        foregroundNode.addChild(eleven)
        
        twelve = createDistanceAtPosition(.Two)
        twelve.position = CGPoint(x: width * 0.5, y: height * 0.42)
        ...
        timerLabel.position = CGPoint(x: width * 0.7, y: height * 0.01)
        timerLabel.text = "Time: "
        foregroundNode.addChild(timerLabel)
        
        timer = SKLabelNode(fontNamed: "ChalkboardSE-Bold")
        timer = SKLabelNode(text: "30")
        timer.fontSize = 30
        timer.fontColor = SKColor.greenColor()
        timer.position = CGPoint(x: width * 0.8, y: height * 0.01)
        ...
        lblScore.position = CGPoint(x: width * 0.575, y: height * 0.01)
        lblScore.text = "0"
        ...
        lblText.position = CGPoint(x: width * 0.35, y: height * 0.01)
        ...
        var r = 0
        if let levelPath = NSBundle.mainBundle().pathForResource("Street1", ofType: "txt") {
    ...
    func createPlayer() -> SKNode {
        ...
        playerNode.position = CGPoint(x: width * 0.025, y: height * 0.05)
    ...
    func createRightArrow() -> SKNode{
        ...
        rightNode.position = CGPoint(x: width * 0.525, y: height * 0.15)
    ...
    func createUpArrow() -> SKNode{
        let upNode = SKNode()
        upNode.position = CGPoint(x: width * 0.475, y: height * 0.2)
    ...
    func createDownArrow() -> SKNode{
        let downNode = SKNode()
        downNode.position = CGPoint(x: width * 0.475, y: height * 0.1)
